...
Follow-up after maintainer commits on 2026-06-11:
...
| M02 | Subscribe uses `match`, `matchExact`, `matchURLPattern`; unknown `match*` names are rejected. | Subscription | MUST | Parse canonical matcher params; reject `topic` outside v8 compat. | #1269 `subscribematchers.go`, `subscribe_notdeprecated_topic_test.go` | OK after follow-up: UI/docs/conformance now use modern params |
...
| M06 | Relative URLPattern patterns/topics use the hub URL as base. | URL Pattern | MUST | Use actual hub URL as base. | #1269 `topicselector.go`; #1273 `hub.go`, `Caddyfile`, `docs/hub/config.md` | OK after follow-up for full hub `resource_identifier`; fallback remains only when no hub URL is configured/derived |
...
| M17 | `typ` must be `at+jwt` or `application/at+jwt`. | Token Validation / RFC 9068 | MUST | Reject other token types. | #1273 `authorization.go` | OK after follow-up; mixed-case `Application/AT+JWT` covered |
| M18 | Accepted JWS algorithms are explicitly allowlisted; `alg=none` is rejected; algorithms are not derived from token. | Token Validation | MUST / MUST NOT | Always pass explicit accepted algorithms to JWT parser. | #1273 `authorization.go`, `hub.go`, `caddy/mercure.go` | OK after follow-up; JWKS/keyfunc path gets asymmetric default allowlist |
...
| M32 | Metadata includes `resource`, `authorization_servers`, RFC 6750 bearer methods only, cookie extension separately, and supported authorization detail types. | Protected Resource Metadata | SHOULD / extension | Advertise header/query, not cookie in bearer methods; use `mercure_cookie`; advertise `authorization_details_types_supported`. | #1273 `resourcemetadata.go` | OK after follow-up |
...
| M36 | Protected resource identifier is an HTTPS URL without a fragment before being published in RFC 9728 metadata. | Protected Resource Metadata / RFC 9728 | Definition / validation requirement for clients | Reject or avoid publishing invalid resource identifiers. | #1273 `hub.go`, `resourcemetadata.go`, `caddy/mercure.go` | Partial after follow-up: non-URL and fragment values rejected; non-HTTPS values still published with warning |
