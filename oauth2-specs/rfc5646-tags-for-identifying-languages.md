...
5. The same variant subtag MUST NOT be used more than once within a
  language tag.
  6. For example, the tag "de-DE-1901-1901" is not valid.
...
1. Use as precise a tag as possible, but no more specific than is
  ...
  2. For example, 'de' might suffice for tagging an email written
    in German, while "de-CH-1996" is probably unnecessarily
    precise for such a task.

  3. Note that some subtag sequences might not represent the
...
3. If a tag or subtag has a 'Preferred-Value' field in its registry
  ...
  4. For example, use 'jbo' for Lojban in preference to the
...
5. [ISO639-2] has defined several codes included in the subtag
  ...
  application.
  6. The 'mul' (Multiple) primary language subtag identifies
  ...
  7. The 'und' (Undetermined) primary language subtag identifies
...
6. Use variant subtags sparingly and in the correct order.  Most
  ...
  For example:
  7. The tag "en-scotland-fonipa" (English, Scottish dialect, IPA
    phonetic transcription) is correctly ordered because
    'scotland' has a 'Prefix' of "en", while 'fonipa' has no
    'Prefix' field.

  8. The tag "sl-IT-rozaj-biske-1994" is correctly ordered: 'rozaj'
...
1. Extension sequences are ordered into case-insensitive ASCII order
  by singleton subtag.
  2. For example, the subtag sequence '-a-babble' comes before
    '-b-warble'.
...
2. If the language tag starts with a primary language subtag that is
  ...
  3. For example, "hak-CN" (Hakka, China) has the primary language
    subtag 'hak', which in turn has an 'extlang' record with a
    'Prefix' 'zh' (Chinese).  The extlang form is "zh-hak-CN"
    (Chinese, Hakka, China).

  4. Note that Step 2 (prepending a prefix) can restore a subtag
