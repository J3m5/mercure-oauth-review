...
1. Collect the set of potentially applicable keys.  Sources of keys
  may include:
  2. Keys supplied by the application protocol being used.

  3. Keys referenced by the "jku" (JWK Set URL) Header Parameter.

  4. The key provided by the "jwk" (JSON Web Key) Header Parameter.

  5. The key referenced by the "x5u" (X.509 URL) Header Parameter.

  6. The key provided by the "x5c" (X.509 certificate chain) Header
    Parameter.

  7. Other applicable keys available to the application.
