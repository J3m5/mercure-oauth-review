...
1. When recipient is the origin server and If-Match is present,
  ...
  2. if true, continue to step 3

  3. if false, respond 412 (Precondition Failed) unless it can be
...
challenge = auth-scheme [ 1SP ( token68 / [ auth-param ( OWS ","
OWS auth-param ) ] ) ]
codings = content-coding / "identity" / ""
comment = "(" ( ctext / quoted-pair / comment ) ")"
complete-length = 1DIGIT
connection-option = token
content-coding = token
credentials = auth-scheme [ 1SP ( token68 / [ auth-param ( OWS ","
OWS auth-param ) ] ) ]
